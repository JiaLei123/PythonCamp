...
stock
    ...
    try:
        if urlPayload[0] == "topUrl":
            children_category = soup.findAll("div", {"class": "filter_line"})
            if children_category and len(children_category) > 1:
                year_category = children_category[1]
                year_list = year_category.findAll('a')
                for year in year_list[1:]:
                    addUrl(url + year['href'], payload=['child_year_page', year.string.strip()])

        if urlPayload[0] == "child_year_page":
            parse_children(soup, urlPayload[1])
            # get next page
            # for debug mode not need get next page data
            if not run_small:
                add_next_page(addUrl, soup, url, urlPayload[1])
...
def add_next_page(addUrl, soup, url, children_age):
    page_num = soup.findAll(attrs={'class': 'page_num'})
    total_page_num = int(page_num[-1].string)
    next_url = url.replace('=0', '=%d')
    for i in range(30, 30 * total_page_num, 30):
        new_next_url = next_url % i
        addUrl(new_next_url, payload=['next_page', children_age])


def parse_children(soup, children_age):
    children_soup = soup.findAll(attrs={'class': 'list_item'})
    for children_soup in children_soup:
        title = ((children_soup.find(attrs={'class': 'figure_title'})).find('a')).string.strip()
        title = re.sub(u'（.*?版）', '', title)
        children_desc = children_soup.find(attrs={'class': 'figure_desc'})
        if children_desc:
            children_relate_words = children_desc.string.strip()
        else:
            children_relate_words = ''

        children_figure = children_soup.find("a", {"class": "figure"})
        data_float = children_figure['data-float']
        children_year, children_type = get_children_detail(data_float)

        children_score = children_soup.find(attrs={'class': 'figure_score'})
        if children_score:
            score_major = children_score.find("em", {"class": "score_l"})
            score_minor = children_score.find("em", {"class": "score_s"})
            score = score_major.string.strip() + score_minor.string.strip()
        else:
            score = ''

        children_play_count = children_soup.find(attrs={'class': 'figure_count'})
        if children_play_count:
            play_count = children_play_count.find("span", {"class": "num"}).string.strip()
        else:
            play_count = ''
        ...
        for item in total_list:
            if len(item) > 0 and title == item[0]:
                need_merge = True
                item[6] = ','.join((item[6], children_age))
                if item[3] < play_count_int:
                    item[3] = play_count_int
                break

        if not need_merge:
            each_list = list()
            each_list.append(title)
            each_list.append(children_relate_words)
            each_list.append(score)
            each_list.append(play_count_int)
            each_list.append(children_year)
            each_list.append(children_type)
            each_list.append(children_age)
            total_list.append(each_list)


def get_children_detail(data_float):
    children_year = ''
    children_type = ''
    try:
        ...
        if response.status_code == 200:
            try:
                children_detail = json.loads(response.text)
            except Exception as e:
                print e
            else:
                children_year = children_detail['c']['year']
                if len(children_detail['typ']) > 0:
                    type_json = children_detail['typ'][0]
                    if isinstance(type_json, list):
                        children_type = ','.join(type_json)
                    else:
                        children_type = type_json
        else:
            print 'fail to get data for: ', data_float
    except Exception as ex:
        print ex
    return children_year, children_type
...
if __name__ == '__main__':
    ...
    parser.add_option(
        '--robots',
        default='/lm/data2/scrapers/zho-CHN/epg/v.qq.com.children/log.inc/'
                'robots.txt.zip',
    ...
    parser.add_option(
        '--badUrlsFile',
        dest='badUrlsFile',
        default='/lm/data2/scrapers/zho-CHN/epg/v.qq.com.children/log.inc/v.qq.com.children.badUrls.lst',
    ...
    if options.robots:
        ...
        myScraper.setRobotsTxt(url='http://v.qq.com/children/',
                               zip=options.robots)
    ...
    myScraper.addUrl(
        u'http://v.qq.com/x/list/children',
        payload=['topUrl']
    )
